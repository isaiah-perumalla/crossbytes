...
impl<'a> CountersInner<'a> {
    ...
    fn tail_intent_counter(&'a self) -> &'a AtomicU64 {
        self.buff
            .get_atomic(TAIL_INTENT_COUNTER_OFFSET as usize)
    }
...
impl<'a> BroadcastTx<'a> {
        ...
        F: Fn(BytesAtomicView) -> usize,
    {
        ...
        if capacity < (record_offset + aligned_record_len) {
            ...
            //we adding padding + data for new tail
            ...
            let mut buffer = self
                .buffer
                .sub_view(0, msg_size + HEADER_SIZE);
            ...
            let mut buffer = self
                .buffer
                .sub_view(record_offset, msg_size + HEADER_SIZE);
...
impl<'a> BroadcastRx<'a> {
    ...
    pub fn receive_next<F>(&mut self, mut f: F) -> Result<u32, RxErr>
    where
        F: FnMut(MsgTypeId, BytesAtomicView),
    {
        ...
        let tail_counter = self.index_inner.tail_counter();
        let tail_intent_counter = self.index_inner.tail_intent_counter();
        let latest_counter = self.index_inner.latest_record_counter();
        ...
        if PADDING_MSD_ID.inner() == msg_id {
            ...
            assert_ne!(
                ...
                "cannot have two consective paddings"
            );
            let next_record_position = record_offset + aligned_record_size as u64;
            ...
            f(MsgTypeId(msg_id), data_buffer);

            let res = self.verify_read(tail_intent_counter, record_size);
            if res.is_ok() {
    ...
    fn verify_read(&self, tail_intent_counter: &AtomicU64, record_size: u32) -> Result<u32, RxErr> {
        //need to check if data read was not overwritten while reading
        //need to ensure reads above this fence happen before susequent reads
        atomic::fence(Acquire);
        let tail_intent_position = tail_intent_counter.load(Acquire);
        let is_valid = (self.cursor + self.buffer.len() as u64) > tail_intent_position;
...
#[cfg(test)]
mod tests {
    use crate::broadcast::{align, BroadcastRx, BroadcastTx, MsgTypeId, RxErr, HEADER_SIZE, TRAILER_SIZE};
